...
void calculate_max_min(float *arr, int n, float *M_val, float *m_val, int *M_loc, int *m_loc)
{
	float max_val;
	float min_val;
	int max_location;
...
int main(int argc, char **argv)
{
	...
	float filler1, filler2, filler3, filler4;
	while ((read = getline(&line, &len, fp)) != -1) {
		/* parse the data */
		rv = sscanf(line, "%f,%f,%f,%f,%f,%f,%f\n", &t[i], &filler1, &filler2, &filler3, &filler4, &x[i], &y[i]);
		if (rv != 7) {
			fprintf(stderr,
					"%s %d \'%s\'. %s.\n",
					"Failed to read line",
	...
	float xmean = 0;
	xmean = calculate_mean(x,N_SAMPLES);
	printf("\nMean value of signal x cross y: %f\n", xmean);

	float xmax_val,xmin_val;
	int xmax_loc,xmin_loc;
	calculate_max_min(x,N_SAMPLES,&xmax_val,&xmin_val,&xmax_loc,&xmin_loc);
	printf("Maximum value of signal x cross y: %f\tLocation: %f\n\n",xmax_val,t[xmax_loc]);
	
	FILE* extrema_file;
	extrema_file = fopen("xcorr_extrema.dat","w");
	fprintf(extrema_file,"%f\t%f\t%f\t%f\t\n",xmax_val,t[xmax_loc],xmin_val,t[xmin_loc]);
	...
	float ymean = 0;
	ymean = calculate_mean(y,N_SAMPLES);
	printf("\nMean value of signal y cross x: %f\n", ymean);

	float ymax_val,ymin_val;
	int ymax_loc,ymin_loc;
	calculate_max_min(y,N_SAMPLES,&ymax_val,&ymin_val,&ymax_loc,&ymin_loc);
	printf("Maximum value of signal y cross x: %f\tLocation: %f\n\n",ymax_val,t[ymax_loc]);

	fprintf(extrema_file,"%f\t%f\t%f\t%f\t\n",ymax_val,t[ymax_loc],ymin_val,t[ymin_loc]);
	fclose(extrema_file);
